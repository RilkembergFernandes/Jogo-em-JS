...
function preload() {
  ...
  snd5 = loadSound(
    "Sons/vitória _ 🔊 efeito sonoro para vídeo🔊 (320 kbps) (mp3cut.net).mp3"
  );
}
...
function setup() {
  ...
  // carro 1 (coordenadas iniciais)
  x = 10;
  y = 348;
  // carro 2 (coordenadas iniciais)
  x2 = 350;
  y2 = 140;
  // carro 3 (coordenadas iniciais)
  x3 = 200;
  y3 = 80;
  // carro 4 (coordenadas iniciais)
  x4 = 100;
  y4 = 40;
  // carro 5 (coordenadas iniciais)
  x5 = 100;
  y5 = 150;
...
function game() {
  ...
  if (i % 2 !== 0) {
    image(patinho[0], xpato, ypato, 30, 30);
  } else {
    image(patinho[1], xpato, ypato, 30, 30);
  }
  //mover o personagem
  ...
  //vendo se pego nos carros se colidir tira vida e tempo e toca um som especifico
  ...
  hitpatocarro3 = collideRectRect(x4, y4, 60, 30, xpato, ypato, 30, 30);
  ...
  // vendo se o tempo chega zero
  tempo++;
  if (tempo == 100) {
    segundos--;
    tempo = 0;
  }
  // fazendo o tempo piscar em vermelho quando se aproxima de 0
  if (segundos <= 5) {
    fill(200, 0, 0);
    text("Tempo " + segundos, 270, 32);
  }
  // mostrnado a tela de gameover quando morrer ou acabar o tempo
  if (vidas <= 0 || segundos <= 0) {
    estado = 3;
    segundos = 10;
    vidas = 3;
    snd4.play();
  }
  // dizer que o jogador está quase lá quando falta poucos pontos para acabar
  if (pontos >= 18) {
    fill(random(0, 200));
    text("Voce esta quase lá!! ", 100, 250);
  }
  // mostrar tela de fase concluida
  if (pontos >= 20) {
    estado = 7;
    snd5.play();
    pontos = 0;
...
function fases() {
  ...
  if (
    mouseX > xdoRetangulo &&
    mouseX < xdoRetangulo + larguradoRetangulo &&
    mouseY > ydoRetangulo &&
    mouseY < ydoRetangulo + alturadoRetangulo
  ) {
    strokeWeight(4);
  ...
  if (
    mouseX > xdoRetangulo2 &&
    mouseX < xdoRetangulo2 + larguradoRetangulo2 &&
    mouseY > ydoRetangulo2 &&
    mouseY < ydoRetangulo2 + alturadoRetangulo2
  ) {
    strokeWeight(4);
...
function keyPressed() {
  if (keyCode === LEFT_ARROW) {
    ...
    rect(xbotao1, ybotao1, largura + 2, altura + 2);
  }
